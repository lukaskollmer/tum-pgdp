...
public class Parser {
    ...
    Declaration parseDecl() {
        ...
        fullDeclaration = fullDeclaration.replace("int ", "");
    ...
    Expression parseExpression() {
        ...
        if (expression != null && tokens[currentPosition].equals("(")) {
            ...
            String functionName = ((Variable)expression).getName();
            log("function name: %s\n", functionName);
            ...
            if (tokens[currentPosition].equals(";")) {
                return new Call(functionName, parameters);
            } else {
                log("something went wrong parsing the function call\n");
                // error. we're in a function call, but didn't manage to parse expressions
        ...
        if (expression == null) {
            if (tokens[currentPosition].equals("-")) { // or currentPosition + 1 ???
                ...
                if (expressionFollowingTheUnop != null) {
                    ...
                    log("after unop: %s (%s)\n", currentPosition, tokens[currentPosition]);
                    return new Unary(Unary.Unop.MINUS, expressionFollowingTheUnop);
                } else {
                    return null;
                }
            } else {
                ...
                if (tokens[currentPosition].equals("(")) {
                    ...
                    // otherwise, we set currentPosition to the token after the closing parens
                    // (this will be picked up below for checking for a potential binop)
                    if (tokens[currentPosition].equals(")") && parseBinop() == null) {
                        currentPosition += 1;
                        return expressionWrappedInParens;
                    } else {
                        currentPosition += 1;

                        // we don't actually use `expression` below, but there's a nonnull check we need to pass when coming from here ¯\_(ツ)_/¯
